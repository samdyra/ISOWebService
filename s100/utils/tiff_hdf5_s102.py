...
def tiff_hdf5_s102(bio, bio_param: Dict[str, Union[str, int]]):
    ...
    with File(bio, 'w') as f:
        ...
        bathy_group_object = f.create_group(
            '/BathymetryCoverage/BathymetryCoverage.01/Group_001')
        grid = bathy_group_object.create_dataset(
            'values', dtype=[('depth', '<f4'), ('uncertainty', '<f4')], shape=shape)
        ...
        f.attrs.create('verticalDatum',
                       data=vertical_datum_dt_type, dtype=vertical_datum_dt)
        ...
        bathy.attrs.create(
            'dataCodingFormat', data=data_coding_format_dt_type, dtype=data_coding_format_dt)
        bathy.attrs.create(
            'commonPointRule', data=common_point_rule_dt_type, dtype=common_point_rule_dt)
        ...
        depth_grid_rounded = numpy.round(depth_grid, decimals=2)
        depth_grid_no_nan = numpy.nan_to_num(depth_grid_rounded, nan=-100000)

        uncert_grid_rounded = numpy.round(uncert_grid, decimals=2)
        uncert_grid_no_nan = numpy.nan_to_num(uncert_grid_rounded, nan=-100000)

        grid[:, 'depth'] = depth_grid_no_nan
        grid[:, 'uncertainty'] = uncert_grid_no_nan
        ...
        batcov = Group_F.create_dataset(
            'BathymetryCoverage', shape=(2,), dtype=dt)

        batcov[0] = ('depth', 'Depth', 'metres', '1000000',
                     'H5T_FLOAT', '-12000', '12000', 'closedInterval')
        batcov[1] = ('uncertainty', 'Uncertainty', 'metres', '1000000',
                     'H5T_FLOAT', '-12000', '12000', 'closedInterval')
